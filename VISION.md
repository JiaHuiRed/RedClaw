...
RedClaw is the AI that actually does things.
...
RedClaw started as a personal playground to learn AI and build something genuinely useful:
an assistant that can run real tasks on a real computer.
It evolved through several names and shells: Warelay -> Clawdbot -> Moltbot -> OpenClaw -> RedClaw.
...
RedClaw runtime code reads the current configuration schema only.
...
Security in RedClaw is a deliberate tradeoff: strong defaults without killing capability.
...
RedClaw has an extensive plugin API.
...
- Code plugins run RedClaw plugin code and are appropriate for deeper runtime extension.
...
RedClaw docs should document core extension points; plugin promotion belongs in ClawHub, preferably under vetted org publishers for official plugins.
...
RedClaw supports MCP as both a server and a runtime integration surface.
...
RedClaw is currently terminal-first by design.
...
RedClaw is primarily an orchestration system: prompts, tools, protocols, and integrations.
TypeScript was chosen to keep RedClaw hackable by default.
