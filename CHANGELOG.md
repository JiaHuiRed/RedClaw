...
- **微信插件版本门禁**：`@tencent-weixin/openclaw-weixin` 要求宿主版本 `>= 2026.3.22`（OpenClaw 日期版本格式），RedClaw 语义版本 `0.3.x` 被解析为 `{year: 0}` 直接拒绝。将既有未接线的 `resolveCompatibilityHostVersion()` 接入插件运行时 `api.runtime.version`——插件面报告兼容日期版本 `2026.6.3`，内部版本号不变。

### 新增

- **微信端到端打通**：插件安装（`plugins install`）→ 扫码登录（`channels login`，用户侧终端执行避免二维码 2 分钟时效中转过期）→ `weixin monitor started`，手机微信与秋秋真实收发验证通过。附带发现：未配置账号的渠道插件启动时自动跳过，账号写入后自动装载。

---

## [0.3.24] - 2026-09-05

> 右面板下半区利用起来：用量 + 记忆组合面板；全面板卡片彩色分区 + 按压浮起手感。

### 新增

- **记忆面板**（新增 `memory.overview` RPC + 面板记忆区）：网关新增只读 `memory.overview` 方法（`operator.read`）——一次返回长期记忆 `MEMORY.md`（大小/字数/预览）+ `memory/` 目录全部 `.md`（按更新时间倒序，前 60 个、前 12 个带正文预览），GUI 右面板用量下半区新增「记忆」区：记忆文件数/总量、长期记忆大小与最近更新、记忆总字数、最近记录时间四张统计卡 + 最近记忆列表（日期标签 + 大小 + 单行预览，点按展开）。
- **彩色卡片分区**：统计卡按语义着色——琥珀=花费、蓝=tokens、桃粉=活跃、绿=命中率环、紫=记忆（新增 violet 12 阶色阶，OKLCH 生成器同步扩展，深浅主题各校准）；卡片底色/描边用 `color-mix` 混合主题变量，深浅主题自动适配。
- **按压浮起手感**：所有统计卡与记忆行统一交互——悬停上浮 + 投影，按下轻缩回弹（`hover:-translate-y-0.5 hover:shadow-md active:scale-[0.97]`）。
- **刷新按钮修复**：原刷新按钮 `setRange(r => r)` 值不变不触发重取，改为共享 `refreshTick` 同时刷新用量与记忆。
