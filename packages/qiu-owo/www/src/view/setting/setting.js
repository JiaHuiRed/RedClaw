...
export default () => {
  ...
  const SettingField = {
    view: ({ attrs }) => {
      ...
      // 聊天背景选择
      if (opt.key === "chatBackground") {
        const bgValue = opt.value || ""
        return m("div", { style: { marginBottom: "2rem" } }, [
          m("label", { style: { display: "block", color: getColor('gray_1').front, marginBottom: "1rem", fontWeight: "bold", fontSize: "1.1rem" } }, trs("设置界面/字段/" + (opt.name || "聊天背景"))),
          m("div", { style: { display: "flex", gap: "1rem", alignItems: "flex-start", flexWrap: "wrap" } }, [
            // 预览区
            m("div", {
              style: {
                width: "200px", height: "120px", borderRadius: "0.8rem",
                border: `2px dashed ${getColor('gray_1').front}33`,
                background: getColor('gray_3').back,
              ...
              m("span", { style: { color: getColor('gray_1').front, fontSize: "0.85rem", opacity: 0.5 } }, "未设置背景")
            ]),
            // 操作按钮
            m("div", { style: { display: "flex", flexDirection: "column", gap: "0.6rem" } }, [
              ...
              m("label", {
                style: {
                  ...
                  background: getColor('main').back, color: getColor('main').front,
                ...
                m("input", {
                  ...
                  onchange: async (e) => {
                    ...
                    try {
                      const res = await fetch("/api/attachments/set", { method: "POST", body: formData })
              ...
              bgValue ? m("button", {
                style: {
                  ...
                  border: "1px solid #ff6b6b44", cursor: "pointer",
                  fontSize: "0.9rem", transition: "all 0.2s"
                },
                onmouseover: (e) => { e.currentTarget.style.background = "#ff6b6b11" },
