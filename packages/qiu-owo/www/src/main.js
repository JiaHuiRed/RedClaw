...
(async () => {
  try {
    ...
    // 初始化 Gateway 连接（如果启用了）
    const gwEnable = await settingData.options.get("gateway_enable")
    if (gwEnable === 1) {
      const gwUrl = await settingData.options.get("gateway_url") || "ws://127.0.0.1:18789"
      const gwToken = await settingData.options.get("gateway_token") || ""
      ioSocket.initGateway(gwUrl, gwToken)
      console.log("[Gateway] 从配置初始化: " + gwUrl)
    }
