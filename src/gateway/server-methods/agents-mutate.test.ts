...
describe("agents.create", () => {
  ...
  it("writes identity to config without creating IDENTITY.md", async () => {
    const { promise } = makeCall("agents.create", {
      name: "Plain Agent",
      workspace: "/tmp/ws",
      emoji: "🤖",
    ...
    expect(configOptions.identity).toMatchObject({
      name: "Plain Agent",
      emoji: "🤖",
      avatar: "https://example.com/avatar.png",
    });
    // IDENTITY.md 已废弃（fork 决策）：identity 只进配置，不写工作区文件
    const identityWrites = mocks.rootWrite.mock.calls.filter(
      ([, relativePath]) => relativePath === "IDENTITY.md",
    );
...
describe("agents.update", () => {
  ...
  it("updates identity in config without touching IDENTITY.md", async () => {
    const { promise } = makeCall("agents.update", {
      agentId: "test-agent",
      emoji: "🤖",
